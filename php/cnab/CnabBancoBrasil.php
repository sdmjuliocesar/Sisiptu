...
class CnabBancoBrasil extends CnabAbstract {
    ...
    private function gerarRegistroTitulo(array $dadosBanco, array $titulo, int $sequencial): string {
        ...
        $linha .= $this->formatarAlfanumerico($titulo['cliente_nome'] ?? '', 25); // 165-189 - Identificação do sacado
        $linha .= str_repeat(' ', 40); // 190-229 - Endereço do sacado
        $linha .= str_repeat(' ', 12); // 230-241 - Primeira mensagem
        $linha .= $this->formatarAlfanumerico($titulo['cep'] ?? '', 8); // 242-249 - CEP
